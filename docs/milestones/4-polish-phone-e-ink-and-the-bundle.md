...
- **Trade-off:** the embedded bundle roughly doubles in file count and the binary grows
  ...
  other build outputs under a kilobyte, nine of them per-language parsers and the other two
  a runtime shim and a one-line markdown re-export, keep the clause true for a different
  reason.
...
- **`index.html` is no longer under the compression threshold.** The precompression
  ...
  **1,457 bytes**, over the 1 KB floor, so the build now writes `index.html.br` (595 B) and
  ...
  different reason: eleven other build outputs under a kilobyte still ship with no
  siblings — nine per-language parsers, a runtime shim and a one-line markdown re-export. This is a statement one task in this milestone made true and a later one in the
...
| The binary is about 1.11–1.13 MiB larger (+7.1–7.2%; the two figures are the author's build and the reviewer's, which differed in their build flags) for the brotli and gzip copies it now embeds, and the first `Ctrl+E` of a page costs about 25–30 ms more than it did | [#59](https://github.com/davison/md-notes/issues/59#issuecomment-5656189477), [#59](https://github.com/davison/md-notes/issues/59#issuecomment-5656192500) |
...
| The first `Ctrl+E` has four medians: **55.6 ms** (48.2–75.3) on #59, **85 ms** (74–100) taken for this record, **87 ms** (60–94) by QA, and **103 ms** by round one of #71's review — whose round two then measured **86.0 ms** in the same environment, so the spread is load rather than hardware. The decision set "about 100 ms" as its own condition and one sample is over it, which is why [the introduction](../introduction.md#editing) states the shape rather than a number. How far that shape is corroborated is worth stating exactly: only **two** runs measured all three cells — this record's 85 / 85 / 7 and #71's round two at 86.0 / 85.1 / 6.9 — and they agree that a cached new page costs the same as a cold one and a held toggle is twelve times quicker. #59's table has no cached-new-page row and no held figure at all, round one of #71's review reported only the cold median, and QA's held toggle is 17 ms against 87 cold, which is five times rather than twelve | [#55](https://github.com/davison/md-notes/issues/55#issuecomment-5658104406), finding 8; [#71](https://github.com/davison/md-notes/pull/71#issuecomment-5658292677), finding 1 |
...
| `TestBurstIsOneBatch` in `internal/watch` is still flaky in CI. It recurred **four times** during this milestone — once on PR #43's CI, once on PR #65's and twice on PR #71's — and **three of the four were on branches carrying no Go at all**, which is what rules out a regression and leaves the debounce race on a loaded runner. Each passed on a re-run and none reproduces locally. It is the strongest candidate for an early task in the next milestone | [#46](https://github.com/davison/md-notes/issues/46) |
