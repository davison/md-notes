...
| [#92](https://github.com/davison/md-notes/issues/92) | the review of this record, [#90](https://github.com/davison/md-notes/pull/90#issuecomment-5703126600) | The deleted-on-disk banner still names "another tool" as the way back, where **New note** can now recreate the file — with a possible step further, a button on the banner opening the create prompt pre-filled with the deleted note's path and the draft as its body |
...
| The deleted-on-disk banner still tells the reader to "recreate the file with another tool" (`ui/src/note-pane.tsx`). It is not false — the *editor* still cannot recreate a note — but **New note** can, and the introduction now says so where the string does not | [#92](https://github.com/davison/md-notes/issues/92), from [#90](https://github.com/davison/md-notes/pull/90#issuecomment-5703126600) finding 6 |
