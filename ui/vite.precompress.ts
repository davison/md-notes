 ...
 */
const compressible = /\.(css|html|js|json|map|svg|webmanifest)$/;
 ...
 *
 * `only` narrows the sweep to the files one pass emitted. The service
 * worker is a second Vite pass into the same directory (ui/vite.sw.config.ts)
 * and would otherwise recompress the whole bundle a second time for the sake
 * of one file.
 */
export function precompress(only?: RegExp): Plugin {
  let dir = "";
  return {
    ...
    closeBundle() {
      for (const entry of readdirSync(dir, { recursive: true, withFileTypes: true })) {
        ...
        if (only && !only.test(relative(dir, file))) continue;
